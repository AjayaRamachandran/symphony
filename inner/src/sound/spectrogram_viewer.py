...
def draw_view(
    ...
    title_text = font.render(
        "Average Spectrum (100-2000 Hz, log frequency) - Hover for values",
        True,
        text_color,
    )
    ...
    for freq, amp in zip(visible_freqs, visible_amps_norm):
        ...
        y = amp_to_y(float(amp), plot_rect.top, plot_rect.height)
        points.append((x, y))
    if len(points) > 1:
    ...
    while running:
        ...
        if plot_rect.collidepoint(mouse_x, mouse_y):
            ...
            hover_text = (
                ...
                f"freq={freq_exact:7.2f} Hz | amplitude={amp:7.4f}"
            )
        else:
            hover_freq = None
            hover_text = (
                f"window={window_start_s:4.2f}-{window_start_s + window_duration_s:4.2f}s | "
                "Move mouse over graph. Left/Right arrows shift by 0.1s."
            )
        ...
        for event in events:
            if (
                ...
                root_freq = float(frequencies[root_idx])

                requested_upper = 2.0 * root_freq
                available_upper = float(np.max(frequencies))
                view_min_freq = root_freq
                view_max_freq = min(requested_upper, available_upper)

                if view_max_freq <= view_min_freq:
                    view_max_freq = view_min_freq * 1.01

                sample_freqs = np.geomspace(root_freq, root_freq * 2.0, num=30).astype(np.float32)
                sampled_amps = np.interp(sample_freqs, frequencies, average_amplitude_raw)
                ratio_to_amp = {
                    float(freq / root_freq): float(amp)
                    for freq, amp in zip(sample_freqs, sampled_amps)
                }

                print("\n# Root frequency selected")
                ...
                hover_text = (
                    f"window={window_start_s:4.2f}-{window_start_s + window_duration_s:4.2f}s | "
                    f"root={root_freq:7.2f} Hz | showing {view_min_freq:7.2f}-{view_max_freq:7.2f} Hz"
                )
