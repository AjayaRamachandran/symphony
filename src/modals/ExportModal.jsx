...
function ExportModal({onClose, onComplete}) {
  ...
  return (
    <>
      ...
      text-style='display' onClick={(format === '' || destination === '' || projectName === '') ? null : async () => { await finish(selectedFolder.value); setGlobalDirectory(selectedFolder.value); onClose() }}>
        Export
      </button>
    </>
  );
