...
app.whenReady().then(() => {
  ...
  ipcMain.handle(
    "save-directory",
    async (event, { destination, projectName, sourceLocation }) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (event, fileName, fileLocation = null) => {
      try {
        if (!fs.existsSync(RECENTLY_VIEWED_PATH)) {
        ...
        recentlyViewed = recentlyViewed.filter((item) => {
          if (fileLocation) {
            return !(
              item.name === fileName && item.fileLocation === fileLocation
            );
          }
